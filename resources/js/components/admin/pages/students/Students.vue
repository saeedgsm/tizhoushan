        ...
        <div class="row align-items-center">
          ...
          <div class="col-md-8">
            <div class="float-right">
              ...
              <router-link
                :to="{ name: 'StudentQuickRegister' }"
                class="btn btn-light btn-rounded"
              >
                <i class="dripicons-plus mr-1"></i>
                <strong>ثبت نام سریع</strong>
              </router-link>
            </div>
          </div>
        </div>
                ...
                >
                  <template slot="table-row" slot-scope="props">
                    <span v-if="props.column.field === 'actions'">
                      <button
                        @click="editRow(props.row.actions)"
                      ...
                      <button
                        @click="deleteRow(props.row.actions, props.index)"
...
export default {
  ...
  methods: {
    fetchActiveCourseStudents() {
      Swal.fire({
        icon: "info",
        text: "مدیریت محترم اطلاعات دانش آموزان در حال بارگزاری می باشد! لطفا چند لحظه صبر کنید!",
        confirmButtonText: "اوکی ",
      });
      Vue.http.get("api/admin/students-all").then(
        (response) => {
    ...
    deleteRow(id, index) {
      Swal.fire({
        ...
        if (result.value) {
          Vue.http.delete("api/admin/active-course-student/delete/" + id).then(
            (response) => {
          ...
          Swal.fire(
            "حذف شد!",
            "اطلاعات دانش آموز با موفقیت حذف گردید!",
            "success"
          );
        } else {
          Swal.fire(
            "کنسل شد",
            "اطلاعات دانش آموز مورد نظر هنوز سالم است",
            "info"
          );
        }
      });
    ...
    onRowDoubleClick(params) {
        this.$router.push({ name: "StudentShow", params: { id: params.row.actions } });
